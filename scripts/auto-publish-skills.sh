...
LOG_FILE="/root/clawd/auto-publish-$(date +%Y%m%d-%H%M%S).log"
PUBLISHED_SKILLS=()
FAILED_SKILLS=()
ALREADY_PUBLISHED=()
...
publish_skill() {
    ...
    # Create .clawdhub/lock.json to fix workdir resolution
    mkdir -p .clawdhub
    echo '{"version": 1, "skills": {}}' > .clawdhub/lock.json
    ...
    if CLAWDHUB_WORKDIR="$skill_dir" clawdhub publish . \
