...
main() {
    log "=========================================="
    log "📦 批量上传 Skills 到 ClawdHub（修复版）"
    log "=========================================="
    log ""
    ...
    for skill_file in "$SKILLS_DIR"/*.skill; do
        ...
        if [ ! -f "$skill_md_path" ]; then
            log_warn "SKILL.md 不存在，跳过: $skill_name"
            cd - > /dev/null
            rm -rf "$temp_dir"
            FAILED_COUNT=$((FAILED_COUNT + 1))
            FAILED_SKILLS+=("$skill_name: SKILL.md 不存在")
            continue
        fi
        ...
        # 上传
        cd "$temp_dir/$(dirname "$skill_md_path")"
        
        log_info "  上传: $skill_name"
