#!/usr/bin/env python3
"""
Visual AI Prompts Collector V3
改进版：直接使用 web_search 和 web_fetch 工具
"""
...
# 注意：这个脚本需要作为 Clawdbot 的一部分运行，才能访问工具
# 如果独立运行，需要通过 API 或其他方式调用工具
    ...
    try:
        result = requests.get(
            ...
            headers={
                ...
                "X-Subscription-Token": "BSA7KQ8tFyVg8yvE7lX7yH6yR6yJ6yN6yO6yP6yQ6yR6yS6yT6yU6yV6"
            },
            timeout=30
        )
...
def search_visual_ai_prompts():
    ...
    for category, category_queries in queries.items():
        ...
        for query in category_queries:
            print(f"  - {query}")
            search_results = search_brave(query, count=5)

            if search_results:
                for result in search_results:
                    # 获取完整页面内容
                    print(f"    Fetching full content from {result.get('url', '')[:60]}...")
                    full_content = fetch_page_content(result.get('url', ''))
...
def main():
    """主函数"""
    print("="*60)
    print("🚀 Starting Visual AI Prompts Collection V3")
...
"""
    ...
    # 保存报告
    report_path = DATA_DIR / f"fix-report-{datetime.now(timezone.utc).strftime('%Y-%m-%d-%H%M')}.md"
