...
"""
    ...
    metadata = {
        ...
        "keywords": [
            "ai",
            "prompt",
            actual_type.lower().replace(" ", "-").replace("_", "-")
        ],
        "author": "Clawdbot",
        "license": "MIT",
        "engines": {
...
def main():
    ...
    # 检查输入文件
    image_prompts_file = os.path.join(PROMPTS_DIR, "image-prompts.jsonl")
    video_prompts_file = os.path.join(PROMPTS_DIR, "video-prompts.jsonl")
    ...
    with open(log_file_path, 'w', encoding='utf-8') as log_file:
        ...
        if os.path.exists(image_prompts_file):
            input_files.append(("image", image_prompts_file))
        if os.path.exists(video_prompts_file):
            input_files.append(("video", video_prompts_file))
        if os.path.exists(general_prompts_file):
            input_files.append(("general", general_prompts_file))
