...
description: [DEPRECATED] 请使用 x-prompt-hunter + prompt-to-skill-converter 架构。此技能已废弃，功能已整合到数据发现层和转换发布层。
version: 1.0.0
author: Clawdbot
license: MIT
tags: [automation, prompts, workflow, clawdhub, publishing, deprecated]
category: automation
---
...
⚠️ **请使用 x-prompt-hunter + prompt-to-skill-converter 架构**
...
- **废弃时间**：2026-02-02
- **替代方案**：x-prompt-hunter（数据发现层） + prompt-to-skill-converter（转换发布层）
- **保留原因**：作为参考文档，了解旧的架构设计

### 迁移指南

1. **收集功能**：迁移到 `x-prompt-hunter/src/github_hf_fetcher.py`
2. **转换功能**：迁移到 `prompt-to-skill-converter/scripts/convert-prompts-to-skills.py`
3. **发布功能**：使用 `clawdhub publish --registry https://www.clawhub.ai/api`
4. **编排功能**：使用 cron 或 sessions_spawn 进行编排
